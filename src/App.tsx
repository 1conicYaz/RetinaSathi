...
function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <main className="app-page"><header className="site-header"><div className="header-inner"><Brand/><nav className="site-nav" aria-label="Primary navigation"><NavLink to="/">Overview</NavLink>{viewer.isAuthenticated ? <><NavLink to="/screen">New screening</NavLink><NavLink to="/history">History</NavLink></> : null}</nav><div className="header-actions"><span className="connectivity"><i/>{online ? 'InsForge live' : 'Offline / local'}</span>{isLoading ? null : viewer.isAuthenticated ? <button className="button button--ghost" type="button" onClick={() => void signOut()}>Sign out</button> : <Link className="button button--primary button--small" to="/auth/sign-in">Sign in</Link>}</div></div></header>{!online ? <div className="offline-banner"><AppIcon name="wifi" size={16}/>Internet unavailable. A running local model can still analyze; secure cloud saves will queue in this browser.</div> : null}{children}<footer className="site-footer"><div><Brand/><p>Human-in-the-loop diabetic-retinopathy screening support for frontline health workers.</p></div><div><span>Powered by InsForge</span><span>Research prototype · SIH26038</span><ThemeToggle/></div></footer></main>;
}

function HomePage() {
  const localModel = import.meta.env.VITE_INFERENCE_MODE?.trim() === 'local';
  const v34 = import.meta.env.VITE_MODEL_GENERATION?.trim() === 'v3.4'
    || import.meta.env.VITE_INFERENCE_MODE?.trim() === 'cloud';
  return <AppLayout><div className="home-page"><section className="hero"><div className="hero-copy"><p className="eyebrow">Human-in-the-loop screening · Research prototype</p><h1>Screen retinal images. <em>Prioritize human review.</em></h1><p className="hero-lead">RetinaSathi helps frontline health workers check image quality, review V3.4 referral evidence, and organize specialist follow-up.</p><div className="hero-actions"><Link className="button button--primary" to="/screen">Start a screening <AppIcon name="arrow" size={18}/></Link><a className="button button--secondary" href="#how-it-works">See how it works</a></div><div className="trust-row"><span><AppIcon name="shield" size={17}/>Private patient records</span><span><AppIcon name="wifi" size={17}/>Low-bandwidth workflow</span></div></div><div className="hero-visual" aria-label="RetinaSathi workflow illustration"><div className="retina-orbit"><div className="retina-disc"><span/><i/><b/></div><div className="scan-line"/></div><div className="floating-card floating-card--grade"><small>DR severity</small><strong>Grade 2</strong><span>Moderate · Refer</span></div><div className="floating-card floating-card--quality"><span className="status-dot"/><div><small>Image quality</small><strong>Suitable for screening</strong></div></div></div></section>
  <section className="impact-strip"><div><strong>0–4</strong><span>DR severity grading</span></div><div><strong>Calibrated</strong><span>Referable-DR score</span></div><div><strong>Explicit</strong><span>Module readiness</span></div><div><strong>RLS</strong><span>User-isolated records</span></div></section>
  <section className="how-section" id="how-it-works"><div className="section-intro"><p className="eyebrow">Designed around clinical action</p><h2>From image quality to referral support.</h2><p>Each result combines image quality, disease severity, uncertainty, and explicit model limitations in one workflow.</p></div><div className="feature-grid"><article><span className="feature-index">01</span><AppIcon name="scan" size={27}/><h3>Quality before grading</h3><p>Ungradeable captures stop inference and receive simple retake instructions.</p></article><article><span className="feature-index">02</span><AppIcon name="eye" size={27}/><h3>Two DR signals</h3><p>V3.4 returns a calibrated five-grade result and a separate referable-DR score.</p></article><article><span className="feature-index">03</span><AppIcon name="alert" size={27}/><h3>Clear limitations</h3><p>The cloud result states when an attention map or DME estimate is unavailable.</p></article><article><span className="feature-index">04</span><AppIcon name="shield" size={27}/><h3>Protected records</h3><p>InsForge Auth, row-level security, private storage, and an authenticated inference proxy isolate operator records.</p></article></div></section>
  <section className="model-card-section"><div><p className="eyebrow">Transparent by design</p><h2>An honest model card is part of the product.</h2><p>{v34 ? `${localModel ? 'Local' : 'Azure-hosted'} V3.4 is a calibrated research screening candidate. Its reported results are source-validation evidence, not prospective clinical validation, and every output requires clinician review.` : 'The local model is a research candidate with calibrated DR probabilities and visual influence evidence. Every result still requires clinician review.'}</p></div><dl><div><dt>Dataset</dt><dd>{v34 ? 'APTOS + IDRiD + DeepDRiD' : 'APTOS + IDRiD'}</dd></div><div><dt>Architecture</dt><dd>{v34 ? 'Partial DINOv2-S/14' : 'EfficientNet-B3'}</dd></div><div><dt>Outputs</dt><dd>{v34 ? 'Referable DR + calibrated five-grade probabilities' : 'DR + calibration + visual evidence'}</dd></div><div><dt>Deployment</dt><dd>{localModel ? 'Local MPS / CPU' : 'Azure ONNX CPU'}</dd></div></dl></section></div></AppLayout>;
}
