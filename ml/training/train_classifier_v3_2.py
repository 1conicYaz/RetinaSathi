"""Train RetinaSathi V3.2 with source-grade balancing and three screening heads."""
...
from ml.models import coral_loss, load_official_dinov2_vits14, ordinal_probabilities
...
def run_tiny_overfit(
    ...
    model, foundation = load_official_dinov2_vits14(
        Path(str(config["experiment"]["foundation_provenance"])),  # type: ignore[index]
        dropout=0.0,
    )
    model = model.to(device)
    optimizer = torch.optim.AdamW(model.trainable_parameters(), lr=5e-4, weight_decay=0.0)
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/classifier_v3_2.yaml"))
    ...
    if int(config["preprocessing"]["input_size"]) % 14:
        raise SystemExit("V3.2 DINOv2 input size must be divisible by 14")
    if args.epochs:
