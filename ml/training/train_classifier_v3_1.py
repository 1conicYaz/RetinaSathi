"""Train RetinaSathi V3.1 with source-grade balancing and three screening heads."""
...
from ml.models import MultiHeadDRClassifier, coral_loss, load_v3_0_weights, ordinal_probabilities
...
def run_tiny_overfit(
    ...
    initial_checkpoint = Path(str(config["experiment"]["initial_checkpoint"]))  # type: ignore[index]
    transfer = load_v3_0_weights(model, initial_checkpoint, sha256(manifest))
...
def full_training(
    ...
    initial_checkpoint = Path(str(experiment["initial_checkpoint"]))
    transfer = load_v3_0_weights(model, initial_checkpoint, sha256(manifest))
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/classifier_v3_1.yaml"))
    ...
    if args.initial_checkpoint is not None:
        config["experiment"]["initial_checkpoint"] = str(args.initial_checkpoint)
