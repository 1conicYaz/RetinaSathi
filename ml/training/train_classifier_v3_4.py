"""Train RetinaSathi V3.4 by partially fine-tuning DINOv2-S/14."""
...
from ml.models import coral_loss, load_partial_dinov2_vits14, ordinal_probabilities
...
def load_v32_transfer(model: nn.Module, checkpoint_path: Path, manifest_digest: str) -> dict[str, object]:
    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"V3.2 transfer checkpoint is missing: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if checkpoint.get("manifest_sha256") != manifest_digest:
        raise ValueError("V3.2 transfer checkpoint was trained with a different manifest")
    incompatible = model.load_state_dict(checkpoint["model"], strict=True)
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise ValueError(f"V3.2 transfer checkpoint is incompatible: {incompatible}")
    return {
...
def differential_optimizer(
    model: nn.Module,
    backbone_learning_rate: float,
    head_learning_rate: float,
    weight_decay: float,
) -> torch.optim.AdamW:
    backbone_parameters = list(model.backbone_trainable_parameters())  # type: ignore[attr-defined]
    head_parameters = list(model.head_parameters())  # type: ignore[attr-defined]
    if not backbone_parameters or not head_parameters:
        raise ValueError("V3.4 requires trainable backbone and head parameter groups")
    return torch.optim.AdamW(
        [
            {"params": backbone_parameters, "lr": backbone_learning_rate, "name": "backbone"},
...
def run_tiny_overfit(
    ...
    model, foundation = load_partial_dinov2_vits14(
        ...
        dropout=0.0,
        unfreeze_blocks=int(training["unfreeze_blocks"]),
    )
    transfer = load_v32_transfer(
        model,
        Path(str(config["experiment"]["initial_checkpoint"])),  # type: ignore[index]
        sha256(manifest),
    )
    model = model.to(device)
    optimizer = differential_optimizer(
        model,
        float(training["tiny_overfit_backbone_learning_rate"]),
        float(training["tiny_overfit_head_learning_rate"]),
        0.0,
    )
...
def full_training(
    ...
    transfer = load_v32_transfer(model, Path(str(experiment["initial_checkpoint"])), sha256(manifest))
    model = model.to(device)
    optimizer = differential_optimizer(
        model,
    ...
    for epoch in range(1, int(training["epochs"]) + 1):
        ...
        record = {
            "epoch": epoch,
            "learning_rates": {
                "backbone": scheduler.get_last_lr()[0],
                "heads": scheduler.get_last_lr()[1],
            },
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/classifier_v3_4.yaml"))
    ...
    if int(config["preprocessing"]["input_size"]) % 14:
        raise SystemExit("V3.4 DINOv2 input size must be divisible by 14")
    if args.epochs:
        ...
        json.dumps(
            {
                ...
                "initial_checkpoint": str(config["experiment"]["initial_checkpoint"]),
                "initial_checkpoint_sha256": sha256(Path(config["experiment"]["initial_checkpoint"])),
                "mode": (
                    "tiny_overfit"
