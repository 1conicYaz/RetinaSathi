...
V3.4 does **not** currently provide validated DME, lesion, vessel, optic-disc, fovea or explanation maps. The V3.2 four-class lesion model can be shown separately as an **experimental candidate overlay** in the MATLAB prototype. Its internal full-image mean Dice is 0.3937 on 11 validation images; visible border and optic-disc false positives remain. The overlay does not decide referral, confirm lesions, or explain the V3.4 grade. The earlier V3.4 explanation map failed technical validation and remains disabled. See [the lesion implementation report](LESION_SEGMENTATION_V3_IMPLEMENTATION.md).
...
- Does the heatmap locate lesions? No. V3.4 XAI is disabled because the previous map failed technical validation. The separate V3.2 overlay marks experimental lesion candidates that require clinician confirmation.
...
4. V3.4 XAI is unavailable. The optional V3.2 lesion candidate overlay has false positives and is not validated lesion localization.
...
- `docs/SIH26038.md` — requirement-by-requirement scope and limits
...
- `docs/MATLAB_SIMULINK_USER_GUIDE.md` — step-by-step prototype instructions
